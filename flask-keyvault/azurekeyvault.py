...
class AzureKeyVaultManager(object):
    ...
    def getSecretFromKeyVault(self, secretName, secretKey, keyVault=None):
        if keyVault is None:
            keyVault=self.key_vault

        endpoint = 'https://{0}.vault.azure.net/secrets/{1}/{2}?api-version=2015-06-01'.format(keyVault, secretName, secretKey)
